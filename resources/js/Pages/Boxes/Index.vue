            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                ...
                <div
                    ...
                    <div v-if="successMessage"
                        class="bg-green-500/20 border border-green-400/30 text-green-100 p-4 rounded-lg backdrop-blur-sm mb-6">
                        <p class="font-bold">Éxito</p>
                        <p>{{ successMessage }}</p>
                    </div>
                    <div v-if="errorMessage"
                        class="bg-red-500/20 border border-red-400/30 text-red-100 p-4 rounded-lg backdrop-blur-sm mb-6">
                    ...
                    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" :key="boxesKey">
                        <div v-for="box in boxes" :key="box.id" @click="viewBox(box.id)"
                            ...
                            <div v-if="box.servicio_en_curso"
                                class="mt-4 p-3 bg-indigo-500/10 rounded-md border border-indigo-400/20 text-sm">
                                <p class="text-indigo-100 font-semibold">Servicio en Curso:</p>
                                <p class="text-indigo-200">Vehículo: {{ box.servicio_en_curso.vehiculo?.marca }} {{
                                    box.servicio_en_curso.vehiculo?.modelo }} ({{
                                        box.servicio_en_curso.vehiculo?.patente }})</p>
                                <p class="text-indigo-200">Tipo de Lavado: {{
                                    box.servicio_en_curso.tipo_lavado?.nombre_lavado }}</p>
                                <p class="text-indigo-200">Inició: {{ new
                                    Date(box.servicio_en_curso.fecha_hora_inicio).toLocaleString() }}</p>
                                <p class="text-indigo-200">Asignado por: {{ box.servicio_en_curso.administrador?.name }}
                                </p>
                            </div>
                            <div v-else
                                class="mt-4 p-3 bg-purple-500/10 rounded-md border border-purple-400/20 text-purple-200 text-sm">
                                No hay servicio en curso.
                            </div>
                            <div class="mt-4 flex space-x-2">
                                <Link :href="route('boxes.edit', box.id)" :class="{
                                ...
                                }" :disabled="box.estado === 'ocupado'" @click.stop>
                                <span v-if="box.estado !== 'ocupado'" class="relative z-10 flex items-center gap-1">✏️
                                    Editar</span odel>
                                <span v-else>✏️ Editar</span>
                                <div v-if="box.estado !== 'ocupado'"
                                    class="absolute inset-0 bg-gradient-to-r from-yellow-400/20 to-orange-400/20 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                                </div>
                                </Link>
                                <button @click.stop="deleteBox(box.id)" :class="{
                                    ...
                                    <span v-if="box.estado !== 'ocupado'"
                                        class="relative z-10 flex items-center gap-1">🗑️ Eliminar</span>
