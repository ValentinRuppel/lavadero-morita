...
<template>
    ...
    <AppLayout :user="user">
        <div class="py-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                ...
                <div class="bg-white/15 backdrop-blur-md rounded-2xl shadow-2xl border border-white/20 p-8 relative overflow-hidden">
                    ...
                    <!-- Servicio en curso -->
                    <div v-if="props.box.servicio_en_curso && props.box.estado === 'ocupado'" class="mt-6 p-4 bg-indigo-500/10 rounded-lg border border-indigo-400/20">
                        <h4 class="text-lg font-semibold text-indigo-100 mb-2">Servicio en Curso</h4>
                        <p class="text-indigo-200 text-sm">Vehículo: {{ props.box.servicio_en_curso.vehiculo?.patente }} {{ props.box.servicio_en_curso.vehiculo?.marca }} {{ props.box.servicio_en_curso.vehiculo?.modelo }} {{ props.box.servicio_en_curso.vehiculo?.anio }}</p>
                        <p class="text-indigo-200 text-sm">Cliente: {{ props.box.servicio_en_curso.vehiculo?.cliente_nombre }}</p>
                        <p class="text-indigo-200 text-sm">Tipo de Lavado: {{ props.box.servicio_en_curso.tipo_lavado?.nombre_lavado }}</p>
                        <p class="text-indigo-200 text-sm">Precio Total: ${{ props.box.servicio_en_curso?.precio_total_servicio }}</p>
                        <p class="text-indigo-200 text-sm">Inició: {{ new Date(props.box.servicio_en_curso.fecha_hora_inicio).toLocaleString() }}</p>
                        <p class="text-indigo-200 text-sm">Asignado por: {{ props.box.servicio_en_curso.administrador?.name }}</p>
                        <div class="mt-4 text-center">
                            <p class="text-xl font-bold text-indigo-100">Tiempo Restante: {{ formatTime(remainingTime) }}</p>
                    ...
                    <div v-else-if="props.box.estado === 'activo'" class="mt-6 p-4 bg-green-500/10 rounded-lg border border-green-400/20">
                        <h4 class="text-lg font-semibold text-green-100 mb-4">Iniciar Nuevo Servicio</h4>
                        <form @submit.prevent="submitStartService" class="space-y-4">
                            <div>
                                <InputLabel for="selected_client_id" value="Seleccionar Cliente" class="text-purple-100" />
                                <select
                            ...
                            <div v-if="selectedClientId && !newClientVehicle">
                                <InputLabel for="vehiculo_id" value="Seleccionar Vehículo" class="text-purple-100" />
                                <select
                                    id="vehiculo_id"
                                    class="bg-white/10 border border-purple-300/20 text-purple-100 rounded-md shadow-sm mt-1 block w-full focus:ring-purple-500 focus:border-purple-500 backdrop-blur-sm"
                            ...
                            <div v-if="selectedClientId && newClientVehicle" class="space-y-4">
                                <div>
                                    <InputLabel for="vehiculo_patente_nuevo" value="Patente del Nuevo Vehículo" class="text-purple-100" />
                                    <TextInput
                                ...
                                <div>
                                    <InputLabel for="vehiculo_marca_nuevo" value="Marca del Vehículo" class="text-purple-100" />
                                    <TextInput
                                ...
                                <div>
                                    <InputLabel for="vehiculo_modelo_nuevo" value="Modelo del Vehículo" class="text-purple-100" />
                                    <TextInput
                                ...
                                <div>
                                    <InputLabel for="vehiculo_anio_nuevo" value="Año del Vehículo" class="text-purple-100" />
                                    <TextInput
                    ...
                    <div class="flex justify-start mt-6 space-x-2">
                        ...
                        <Link
                            :href="route('boxes.edit', props.box.id)"
                            :class="{
                                'group relative bg-yellow-500/20 hover:bg-yellow-500/30 text-yellow-200 hover:text-yellow-100 px-4 py-2 rounded-full transition-all duration-300 transform hover:scale-105 border border-yellow-400/20 backdrop-blur-sm': props.box.estado !== 'ocupado',
                                'text-gray-400 cursor-not-allowed px-4 py-2': props.box.estado === 'ocupado'
                            }"
                            :disabled="props.box.estado === 'ocupado'"
                        >
                            <span class="relative z-10 flex items-center gap-2">✏️ Editar Box</span>
                            <div v-if="props.box.estado !== 'ocupado'" class="absolute inset-0 bg-gradient-to-r from-yellow-400/20 to-orange-400/20 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
                        </Link>
                    </div>
        ...
        <div v-if="showPostServiceOptionsModal" class="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/50 backdrop-blur-sm">
            <div class="bg-white/15 backdrop-blur-md rounded-2xl shadow-2xl border border-white/20 p-6 w-full max-w-md relative">
                <div class="absolute top-2 right-2 w-2 h-2 bg-pink-300/40 rounded-full animate-pulse"></div>
                <h3 class="text-xl font-semibold text-purple-100 mb-4">Servicio Finalizado en Box {{ box.nombre_box }}</h3>
